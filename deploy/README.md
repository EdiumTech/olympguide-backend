...
Каталог загружен в рабочую базу 8 сентября 2026 года: 7 вузов, 379 записей программ и направлений, 206 подразделений, 7 891 правило. API сохраняет исходные условия льгот и ссылки на официальные документы. Правила доступны как на существующих страницах программ/олимпиад, так и целиком через `/api/v1/admissions/rules`.

Повторяемый импорт из проверенного GitHub Release и контракт новых полей описаны в [ADMISSIONS.md](ADMISSIONS.md). Проверка рабочего сервера:

```powershell
python -X utf8 deploy/verify_admissions.py --snapshot ../olympguide-static/data_loader/admissions/snapshots/2026
```
